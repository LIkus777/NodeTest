...
@RunWith(JUnit4::class) // Use AndroidJUnit4
class NodeDataStoreTest {

    @get:Rule
    val mainCoroutineRule = MainCoroutineRule()
    private val testScope = TestScope(mainCoroutineRule.dispatcher)
    private lateinit var dataStore: NodeDataStore.Base
    ...
    fun setUp() {
        ...
        dataStore = NodeDataStore.Base(context, "test_pref_file-$random")
    }
    ...
    @Test
    fun `loadTree returns empty list when no data is saved`() = testScope.runTest {
        val tree = dataStore.loadTree()
        advanceUntilIdle()

        assertEquals(emptyList<Node>(), tree)
    }

    @Test
    fun `saveTree and loadTree correctly save and load data`() = testScope.runTest {
        val nodesToSave = listOf(
            Node("node1id", "node1", "root", emptyList()), Node("node2id", "node2", "root", emptyList())
        )
        dataStore.saveTree(nodesToSave)
        advanceUntilIdle()

        val loadedNodes = dataStore.loadTree()
        advanceUntilIdle()

        assertEquals(nodesToSave, loadedNodes)
    }

    @Test
    fun `saveTree correctly overwrites existing data`() = testScope.runTest {
        val initialNodes = listOf(Node("node1id", "node1", "root", emptyList()))
        dataStore.saveTree(initialNodes)
        advanceUntilIdle()

        val newNodes = listOf(Node("node2id", "node2", "root", emptyList()))
        dataStore.saveTree(newNodes)
        advanceUntilIdle()

        val loadedNodes = dataStore.loadTree()
        advanceUntilIdle()
        assertEquals(newNodes, loadedNodes)
    }

    private fun createTestContext(): Context {
        // create a temporary directory to use for the datastore
        val testDir = File("test-data-test")
        testDir.mkdirs()
